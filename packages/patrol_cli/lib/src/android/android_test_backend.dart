...
class AndroidTestBackend {
  ...
  Future<void> execute(
    ...
    final logProcessor = LogProcessor(
      device.id,
      logFilePath,
      (uri) => _handleObservatoryUri(uri, device),
      _logger,
    );
  ...
  Future<void> _handleObservatoryUri(String url, Device device) async {
    _logger.info('Observatory URI found: $url');
